...
你不会找到现实中的工具或云的代码示例。该领域的工具变化太快，无法保持代码示例的准确性，但本书中的建议将不会那么快过时，并且适用于各种工具。相反，我为虚构的工具写了一些伪代码示例来说明概念。对于示例项目和代码的引用请参见本书的配套网站（https://infrastructure-as-code.com)）。
...
本书还介绍了可操作性主题，如监视和可观察性、日志聚合、身份管理以及在云环境中支持服务所需的其他关注点。书中的内容可以帮助你将这些服务所需的基础结构作为代码进行管理，但具体服务的详细信息，你可以在具体的参考资料中找到。
...
本书是为那些参与提供和使用基础设施来交付和运行软件的人准备的。你可能有系统和基础设施的背景，或者有软件开发和交付的背景。你的角色可能是工程、测试、架构或管理人员。假设你或多或少接触了云或虚拟化基础设施，以及通过代码自动化管理基础设施的工具。
